...
(function () {
  ...
  function SettingsManager(options) {
    ...
    const defaultLanguage = window.i18nManager
    ...
    this.defaultSettings = {
      ...
      sessionTimeoutSeconds: 604800, // 預設一週（秒）
      // 自動執行命令設定
      ...
      self
        ...
        .then(function (serverSettings) {
          if (serverSettings && Object.keys(serverSettings).length > 0) {
            self.currentSettings = self.mergeSettings(
          ...
          if (self.currentSettings.language && window.i18nManager) {
            ...
            if (self.currentSettings.language !== currentI18nLanguage) {
              console.log(
                "🔧 SettingsManager.loadSettings: 同步語言設定到 i18nManager"
              );
              console.log(
                "  從:",
                currentI18nLanguage,
                "到:",
                self.currentSettings.language
              );
   ...
   * 合併設定
   */
  SettingsManager.prototype.mergeSettings = function (
    defaultSettings,
    newSettings
  ) {
  ...
  SettingsManager.prototype.handleLanguageChange = function (newLanguage) {
    ...
    setTimeout(() => {
      this.updatePrivacyLevelDescription(
        this.currentSettings.userMessagePrivacyLevel
      );
    }, 100);
   ...
   * 設定自動提交功能
   */
  SettingsManager.prototype.setAutoSubmitSettings = function (
    enabled,
    timeout,
    promptId
  ) {
  ...
  SettingsManager.prototype.applyLayoutMode = function () {
    const layoutModeInputs = document.querySelectorAll(
      'input[name="layoutMode"]'
    );
    layoutModeInputs.forEach(
      function (input) {
  ...
  SettingsManager.prototype.applyAutoCloseToggle = function () {
    ...
    if (autoCloseToggle) {
      autoCloseToggle.classList.toggle(
        "active",
        this.currentSettings.autoClose
      );
    }
  };
  ...
  SettingsManager.prototype.applyLanguageSettings = function () {
    ...
    if (languageSelect) {
      console.log(
        `🔧 SettingsManager.applyLanguageSettings: 設置 select.value = ${this.currentSettings.language}`
      );
      languageSelect.value = this.currentSettings.language;
      console.log(
        `🔧 SettingsManager.applyLanguageSettings: 實際 select.value = ${languageSelect.value}`
      );
    }
    ...
    languageOptions.forEach(
      function (option) {
        option.classList.toggle(
          "active",
          option.getAttribute("data-lang") === this.currentSettings.language
        );
      }.bind(this)
    );
  };
  ...
  SettingsManager.prototype.applyImageSettings = function () {
    ...
    const imageSizeLimitSelects = document.querySelectorAll(
      '[id$="ImageSizeLimit"]'
    );
    imageSizeLimitSelects.forEach(
      function (select) {
    ...
    const enableBase64DetailCheckboxes = document.querySelectorAll(
      '[id$="EnableBase64Detail"]'
    );
    enableBase64DetailCheckboxes.forEach(
      function (checkbox) {
  ...
  SettingsManager.prototype.applyAutoSubmitSettingsToUI = function () {
    ...
    if (autoSubmitToggle) {
      autoSubmitToggle.classList.toggle(
        "active",
        this.currentSettings.autoSubmitEnabled
      );
    }

    // 更新自動提交超時時間輸入框
    const autoSubmitTimeoutInput =
      Utils.safeQuerySelector("#autoSubmitTimeout");
    if (autoSubmitTimeoutInput) {
      autoSubmitTimeoutInput.value = this.currentSettings.autoSubmitTimeout;
    }

    // 更新自動提交提示詞選擇下拉選單
    const autoSubmitPromptSelect = Utils.safeQuerySelector(
      "#autoSubmitPromptSelect"
    );
    if (autoSubmitPromptSelect) {
      autoSubmitPromptSelect.value =
        this.currentSettings.autoSubmitPromptId || "";
    }
  ...
  SettingsManager.prototype.applySessionHistorySettings = function () {
    // 更新會話歷史保存期限選擇器
    const sessionHistoryRetentionSelect = Utils.safeQuerySelector(
      "#sessionHistoryRetentionHours"
    );
    if (sessionHistoryRetentionSelect) {
      sessionHistoryRetentionSelect.value =
        this.currentSettings.sessionHistoryRetentionHours.toString();
    }
  ...
  SettingsManager.prototype.applyUserMessageSettings = function () {
    // 更新用戶訊息記錄啟用開關
    const userMessageRecordingToggle = Utils.safeQuerySelector(
      "#userMessageRecordingToggle"
    );
    if (userMessageRecordingToggle) {
      userMessageRecordingToggle.checked =
        this.currentSettings.userMessageRecordingEnabled;
    }

    // 更新隱私等級選擇器
    const userMessagePrivacySelect = Utils.safeQuerySelector(
      "#userMessagePrivacyLevel"
    );
    if (userMessagePrivacySelect) {
      userMessagePrivacySelect.value =
        this.currentSettings.userMessagePrivacyLevel;
    }
  ...
  SettingsManager.prototype.applySessionTimeoutSettings = function () {
    // 更新會話超時啟用開關
    const sessionTimeoutEnabled = Utils.safeQuerySelector(
      "#sessionTimeoutEnabled"
    );
    if (sessionTimeoutEnabled) {
      sessionTimeoutEnabled.checked =
        this.currentSettings.sessionTimeoutEnabled;
    }

    // 更新會話超時時間輸入框
    const sessionTimeoutSeconds = Utils.safeQuerySelector(
   ...
   * 更新隱私等級描述文字
   */
  SettingsManager.prototype.updatePrivacyLevelDescription = function (
    privacyLevel
  ) {
    const descriptionElement = Utils.safeQuerySelector(
      "#userMessagePrivacyDescription"
    );
    ...
    switch (privacyLevel) {
      ...
      case "disabled":
        descriptionKey =
          "sessionHistory.userMessages.privacyDescription.disabled";
        break;
      default:
        descriptionKey = "sessionHistory.userMessages.privacyDescription.full";
    }
  ...
  SettingsManager.prototype.setupEventListeners = function () {
    ...
    // 圖片設定 - 大小限制選擇器
    const settingsImageSizeLimit = Utils.safeQuerySelector(
    ...
    // 圖片設定 - Base64 相容模式切換器
    const settingsEnableBase64Detail = Utils.safeQuerySelector(
      "#settingsEnableBase64Detail"
    );
    ...
    if (sessionHistoryRetentionSelect) {
      sessionHistoryRetentionSelect.addEventListener("change", function (e) {
        ...
        if (
          window.MCPFeedback &&
          window.MCPFeedback.app &&
          window.MCPFeedback.app.sessionManager
        ) {
    ...
    // 會話歷史匯出按鈕
    const exportHistoryBtn = Utils.safeQuerySelector(
    ...
    // 清空用戶訊息記錄按鈕
    const clearUserMessagesBtn = Utils.safeQuerySelector(
      "#clearUserMessagesBtn"
    );
      ...
      clearUserMessagesBtn.addEventListener("click", function () {
        ...
        if (confirm(confirmMessage)) {
          if (
            ...
            const success =
              window.MCPFeedback.app.sessionManager.dataManager.clearAllUserMessages();
            if (success) {
    ...
    if (sessionTimeoutEnabled) {
      sessionTimeoutEnabled.addEventListener("change", function () {
        ...
        if (
          window.MCPFeedback &&
          window.MCPFeedback.app &&
          window.MCPFeedback.app.webSocketManager
        ) {
    ...
    if (sessionTimeoutSeconds) {
      sessionTimeoutSeconds.addEventListener("change", function (e) {
        ...
        if (isNaN(seconds) || seconds < 300) {
          ...
          e.target.value = 604800;
          self.set("sessionTimeoutSeconds", 604800);
        } else {
        ...
        console.log(
          "會話超時時間已更新:",
          self.get("sessionTimeoutSeconds"),
          "秒"
        );
