...
(function () {
  ...
  function WebSocketManager(options) {
    ...
    this.maxReconnectAttempts =
      options.maxReconnectAttempts || Utils.CONSTANTS.MAX_RECONNECT_ATTEMPTS;
    this.reconnectDelay =
      options.reconnectDelay || Utils.CONSTANTS.DEFAULT_RECONNECT_DELAY;
    this.heartbeatInterval = null;
    this.heartbeatFrequency =
      options.heartbeatFrequency || Utils.CONSTANTS.DEFAULT_HEARTBEAT_FREQUENCY;
  ...
  WebSocketManager.prototype.connect = function () {
    ...
    const connectingMessage = window.i18nManager
      ? window.i18nManager.t("connectionMonitor.connecting")
      : "連接中...";
  ...
  WebSocketManager.prototype.handleOpen = function () {
    ...
    const connectedMessage = window.i18nManager
      ? window.i18nManager.t("connectionMonitor.connected")
      : "已連接";
  ...
  WebSocketManager.prototype.handleError = function (error) {
    ...
    const connectionErrorMessage = window.i18nManager
      ? window.i18nManager.t("connectionMonitor.connectionError")
      : "連接錯誤";
  ...
  WebSocketManager.prototype.handleReconnection = function (event) {
    ...
    // 檢查是否應該重連
      ...
      console.log(
        Math.round(this.reconnectDelay / 1000) +
          "秒後嘗試重連... (第" +
          this.reconnectAttempts +
          "次)"
      );
      ...
      const reconnectingTemplate = window.i18nManager
        ? window.i18nManager.t("connectionMonitor.reconnecting")
        : "重連中... (第{attempt}次)";
      const reconnectingMessage = reconnectingTemplate.replace(
        "{attempt}",
        this.reconnectAttempts
      );
      ...
      setTimeout(function () {
        console.log(
          "🔄 開始重連 WebSocket... (第" + self.reconnectAttempts + "次)"
        );
        self.connect();
      }, this.reconnectDelay);
    } else if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.log("❌ 達到最大重連次數，停止重連");
      const maxReconnectMessage = window.i18nManager
        ? window.i18nManager.t("connectionMonitor.maxReconnectReached")
        : "WebSocket 連接失敗，請刷新頁面重試";
  ...
  WebSocketManager.prototype.startHeartbeat = function () {
    ...
    console.log(
      "💓 WebSocket 心跳已啟動，頻率: " + this.heartbeatFrequency + "ms"
    );
  };
   ...
   * 更新會話超時設定
   */
  WebSocketManager.prototype.updateSessionTimeoutSettings = function (
    settings
  ) {
